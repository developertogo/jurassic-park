...
module Dinosaurs
  class EditOperationTest < ActiveSupport::TestCase
    ...
    test 'should pass contract validation then calling the service' do
      ...
      contract_mock = mock
      Dinosaurs::EditContract.expects(:new).returns(contract_mock)
      ...
      service_mock = mock
      Dinosaurs::EditService.expects(:new).returns(service_mock)
      service_mock.expects(:call).returns(Dry::Monads::Result::Success.new(true))

      operation = Dinosaurs::EditOperation.new(params: params_mock, doorkeeper_application:).call
    ...
    test 'should return errors if something goes wrong while validating params' do
      service_mock = mock
      Dinosaurs::EditService.expects(:new).returns(service_mock).never

      operation = Dinosaurs::EditOperation.new(params: {}, doorkeeper_application:).call
