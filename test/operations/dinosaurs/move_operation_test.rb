...
module Dinosaurs
  class MoveOperationTest < ActiveSupport::TestCase
    ...
    test 'should pass contract validation then calling the service' do
      ...
      contract_mock = mock
      Dinosaurs::MoveContract.expects(:new).returns(contract_mock)
      ...
      service_mock = mock
      Dinosaurs::MoveService.expects(:new).returns(service_mock)
      service_mock.expects(:call).returns(Dry::Monads::Result::Success.new(true))

      operation = Dinosaurs::MoveOperation.new(params: params_mock, doorkeeper_application:).call
    ...
    test 'should return errors if something goes wrong while validating params' do
      service_mock = mock
      Dinosaurs::MoveService.expects(:new).returns(service_mock).never

      operation = Dinosaurs::MoveOperation.new(params: {}, doorkeeper_application:).call
